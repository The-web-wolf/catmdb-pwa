...
const StationsMap = (props) => {
  ...
  return (
    <div className="GoogleMapContainer">
      <Search location={gContext.location} onSelectedChange={makeCoordSelected} getAtms={getAtms} />
      ...
      {props.foundLocations && (
        <div className={props.showMap ? 'StationCard d-flex' : 'StationCard d-none'} ref={cardsRef}>
          {props.foundLocations.map((result) => (
            <StationCard
              key={result.place_id}
              details={result}
              onSelectedChange={setSelectedAndPan}
              isSelected={gContext.selected && gContext.selected.place_id === result.place_id}
            />
          ))}
        </div>
      )}
      ...
      {props.foundLocations && !props.showMap ? (
        <StationsList foundLocations={props.foundLocations} onMapShow={showMap} />
      ) : (
        'Loading...'
      )}
    </div>
